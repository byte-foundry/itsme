...
export default async (event: FunctionEvent<EventData>) => {
  ...
  try {
    ...
    if (!user) {
      userId = await createGraphcoolUser(api, googleUser.sub, googleUser.email)
    } else {
      userId = user.id
    }
...
async function getGoogleUser(googleToken: string): Promise<GoogleUser> {
  const endpoint = `https://www.googleapis.com/oauth2/v3/tokeninfo?access_token=${googleToken}`
...
async function createGraphcoolUser(api: GraphQLClient, googleUserId: string, email: string): Promise<string> {
  const mutation = `
    mutation createUser($googleUserId: String!, $email: String!) {
      createUser(
